...
proj_info = "VT_Unet"
proj_type = "_single_improve"
if not os.path.exists(f"./{proj_info + proj_type}"):
    os.mkdir(proj_info + proj_type)
...
indices = [int(i) for i in "13 26 39".split(" ")]
lis = [81 - 3, 110-3, 110-3]
for (the_ind, index) in enumerate(indices):
    if not os.path.exists(f"./{proj_info + proj_type}/{index}"):
        os.mkdir(f"./{proj_info + proj_type}/{index}")
    ...
    for num in [lis[the_ind]]:
        plt.figure(figsize=(10, 10), dpi = 1200) # h, w, d = 240, 240, 128
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(f"./{proj_info + proj_type}/{index}/origin_{ind}.png", transparent=True, bbox_inches='tight')
    ...
    for num in [lis[the_ind]]:
        ...
        plt.xticks([])
        plt.yticks([])
        plt.savefig(f"./{proj_info + proj_type}/{index}/pred_{ind}.png", transparent=True, bbox_inches='tight')
    print(num)
