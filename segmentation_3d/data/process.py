...
train_set = {
    ...
    'root': '/home/kathy/dataset/BraTs2020/MICCAI_BraTS_2020_Data_Training',
    'flist': '/home/kathy/projects/TransBTS-main/data/train.txt',
    'has_label': True
}

# test/validation data
valid_set = {
    'root': '/home/kathy/dataset/BraTs2020/MICCAI_BraTS_2020_Data_Validation',
    'flist': '/home/kathy/projects/TransBTS-main/data/valid.txt',
    'has_label': False
}

test_set = {
    'root': 'path to testing set',
    'flist': '/home/kathy/projects/TransBTS-main/data/test.txt',
    'has_label': False
}
...
def process_f32b0(path, has_label=True):
    ...
    if has_label:
        label = np.array(nib_load(path + 'seg.nii.gz'), dtype='uint8', order='C')
    modal = modalities[0]
    arr = np.array(nib_load(path + modal + '.nii.gz'), dtype='float32', order='C')
    images = np.stack(
        [np.array(nib_load(path + modal + '.nii.gz'), dtype='float32', order='C') for modal in modalities],
        -1)  # [240,240,155,4]
    print(images.shape)
...
if __name__ == '__main__':
    doit(train_set)
    doit(valid_set)
    path = 'dataset/BraTs2020/MICCAI_BraTS_2020_Data_Validation/BraTS20_TCIA10_467_1'
    process_f32b0(path)
    # doit(test_set)
