...
def process_f32b0(path, has_label=True):
    ...
    if has_label:
        label = np.array(nib_load(path + 'segmentation.nii.gz'), dtype='uint8', order='C')

    images = np.array(nib_load(path + 'imaging.nii.gz'), dtype='float32', order='C')
...
if __name__ == '__main__':
    # doit(train_set)
    # doit(valid_set)
    path = '/home/bai_gairui/kits19/data/case_00000/'
    process_f32b0(path)
    # doit(test_set)
