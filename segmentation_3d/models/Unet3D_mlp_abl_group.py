...
import os 
import sys 
# os.chdir('/home/bai_gairui/seg_3d/model2/segmentation_3d')
...
class WeightedPermuteMLP_3d(nn.Module):
    ...
    def forward(self, x):
        ...
        h = rearrange(x, "b w h d c -> b w c d h")
        # h = h @ mlp_h
        h = self.mlp_h(h)
        h = rearrange(h, "b w c d h -> b w h d c")
        
        w = rearrange(x, "b w h d c -> b c h d w")
        # w = w @ mlp_w
        w = self.mlp_w(w)
        w = rearrange(w, "b c h d w -> b w h d c")
        
        
        d = rearrange(x, "b w h d c -> b w h c d")
        # d = d @ mlp_d 
        d = self.mlp_d(d)
        
        d = rearrange(d, "b w h c d -> b w h d c")
