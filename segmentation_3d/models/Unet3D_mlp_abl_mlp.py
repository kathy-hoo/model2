...
import sys 
sys.path.append('..')
# E:\segmentation_model\model2\segmentation_3d\models\Unet3D_mlp_abl_mlp.py
# os.chdir(r'E:\segmentation_model\model2\segmentation_3d')
# sys.path.append(r'E:\segmentation_model\model2\segmentation_3d')
...
class WeightedPermuteMLP_3d(nn.Module):
    ...
    def forward(self, x):
        ...
        h = rearrange(x, "b w h d (s n) -> b s n (h w d)", n = N)
        # h = h @ mlp_h
        h = self.mlp_h(h)
        h = rearrange(h, "b s n (h w d) -> b w h d (s n)", h = H, w = W, d= D)
