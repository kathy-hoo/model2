...
if __name__ == "__main__":
    mode = "test"
    proj_info = "Ours-udc"
    checkpoint_path = '/home/bai_gairui/seg_3d/model2/segmentation_3d/logs/resoulution-128-train-Ours-udc/version_13/checkpoints/epoch=405-step=45878.ckpt'
    if 1 == 1:
        train_loader = get_data_loader(batch_size=1 if mode== "test" else 2, mode=mode)
        logger = CSVLogger("logs", name=f"resoulution-128-{mode}-{proj_info}")
        trainer = pl.Trainer(accelerator="gpu", devices=[3],  precision=32, max_epochs=5000, logger=logger, log_every_n_steps=1)
